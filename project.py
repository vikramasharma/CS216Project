...
player_totals_df['poscount'] = player_totals_df.groupby('pos')['player'].nunique()
f_player_totals_df = player_totals_df[player_totals_df['poscount']>30]

f_player_totals_df = player_totals_df.sort_values(by=['player_id', 'season'])


f_player_totals_df['pts_diff'] = f_player_totals_df.groupby('player_id')['pts'].diff()

avg_improvement_by_position = f_player_totals_df.dropna(subset=['pts_diff']).groupby('pos')['pts_diff'].mean()
...
combined_positions = f_player_totals_df[f_player_totals_df['pos'].isin(['SF-SG', 'SG-PF'])]
print(combined_positions[['player', 'season', 'pts', 'pts_diff']])

# Calculate median improvement in points scored by position
median_improvement_by_position = f_player_totals_df.dropna(subset=['pts_diff']).groupby('pos')['pts_diff'].median()

min_games = 20
player_totals_df_filtered = f_player_totals_df[player_totals_df['g'] >= min_games]
